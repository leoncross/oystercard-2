...
describe Journey do
  ...
  it "counts the fare as a completed journey for the minimum fare" do
     subject.entry_station(station)
     subject.exit_station(station)
     expect(subject.journey_fare).to eq 1
  end

  it "ensures a penalty fare of 6 has been given for a failed journey (entry)" do
    subject.entry_station(station)
    expect(subject.journey_fare).to eq 6
  end

  it "ensures a penalty fare of 6 has been given for a failed journey (exit)" do
    subject.exit_station(station)
    expect(subject.journey_fare).to eq 6
  end
end
